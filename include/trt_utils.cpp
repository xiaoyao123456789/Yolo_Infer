...
void prepare_buffer(ICudaEngine* engine, float** input_buffer_device,
  ...
  if (cuda_post_process == "c") {
    ...
    *decode_ptr_host = new float[1 + kMaxNumOutputBbox * cpu_bbox_element];
    std::cout << "[DEBUG] CPU mode: allocated decode_ptr_host for " << (1 + kMaxNumOutputBbox * cpu_bbox_element) << " floats" << std::endl;
    ...
    *decode_ptr_host = new float[1 + kMaxNumOutputBbox * gpu_bbox_element];
    ...
    std::cout << "[DEBUG] GPU mode: allocated decode_ptr_host for " << (1 + kMaxNumOutputBbox * gpu_bbox_element) << " floats" << std::endl;
  }
}
...
void infer(IExecutionContext& context, cudaStream_t& stream, void** buffers,
  ...
  if (cuda_post_process == "c") {
    ...
    memset(decode_ptr_host, 0, sizeof(float) * (1 + kMaxNumOutputBbox * cpu_bbox_element));
    std::cout << "[DEBUG] Using decode_ptr_host directly, size: " << (1 + kMaxNumOutputBbox * cpu_bbox_element) << " floats" << std::endl;
    ...
    std::cout << "[DEBUG] outputDims.nbDims: " << outputDims.nbDims << std::endl;
    for (int i = 0; i < outputDims.nbDims; i++) {
      std::cout << "[DEBUG] outputDims.d[" << i << "]: " << outputDims.d[i] << std::endl;
    }
    ...
    for (int i = 0; i < std::min(100, kOutputSize); i++) {
      if (raw_output_host[i] != 0.0f && !std::isnan(raw_output_host[i]) && !std::isinf(raw_output_host[i])) {
        has_valid_data = true;
        break;
      }
    }
    std::cout << "[DEBUG] raw_output_host contains valid data: " << (has_valid_data ? "yes" : "no") << std::endl;

    // 调用CPU解码函数：将网络原始输出转换为检测框
    std::cout << "[DEBUG] Calling cpu_decode function with parameters:" << std::endl;
    std::cout << "[DEBUG] - model_bboxes: " << model_bboxes << std::endl;
    std::cout << "[DEBUG] - kConfThresh: " << kConfThresh << std::endl;
    std::cout << "[DEBUG] - kMaxNumOutputBbox: " << kMaxNumOutputBbox << std::endl;
    std::cout << "[DEBUG] - cpu_bbox_element: " << cpu_bbox_element << std::endl;
    std::cout << "[DEBUG] - num_class: " << num_class << std::endl;
    
    cpu_decode(raw_output_host, model_bboxes, kConfThresh,
               decode_ptr_host, kMaxNumOutputBbox, cpu_bbox_element, num_class);
    
    std::cout << "[DEBUG] cpu_decode function completed" << std::endl;
    std::cout << "[DEBUG] decode_ptr_host[0] (detection count): " << decode_ptr_host[0] << std::endl;

    // 保存解码后、NMS前的结果用于调试
    std::ofstream decode_file("bin/cpp_decode_output.bin", std::ios::binary);
    if (decode_file.is_open()) {
      decode_file.write(reinterpret_cast<const char*>(decode_ptr_host),
    ...
    // 计算仿射逆变换矩阵
    float scale = std::min(kInputH / (float)kInputH, kInputW / (float)kInputW);  // 这里需要实际图像尺寸
    ...
    cv::Mat s2d = (cv::Mat_<float>(2, 3) << 
        scale, 0, -scale * img_width * 0.5f + kInputW * 0.5f,
        0, scale, -scale * img_height * 0.5f + kInputH * 0.5f);
    ...
    std::cout << "[DEBUG] CPU processing completed, detection count: " << decode_ptr_host[0] << std::endl;
    ...
    if (decode_error != cudaSuccess) {
        std::cout << "[ERROR] GPU解码失败: " << cudaGetErrorString(decode_error) << std::endl;
    } else {
        std::cout << "[DEBUG] GPU解码完成" << std::endl;
    }

    // 保存解码后、NMS前的结果用于调试
    float* decode_before_nms = new float[1 + kMaxNumOutputBbox * gpu_bbox_element];
    CUDA_CHECK(
        cudaMemcpyAsync(decode_before_nms, decode_ptr_device,
                        sizeof(float) * (1 + kMaxNumOutputBbox * gpu_bbox_element),
                        cudaMemcpyDeviceToHost, stream));
    CUDA_CHECK(cudaStreamSynchronize(stream));
    ...
    std::ofstream decode_file("bin/cpp_decode_output.bin", std::ios::binary);
    if (decode_file.is_open()) {
      decode_file.write(reinterpret_cast<const char*>(decode_before_nms),
                        sizeof(float) * (1 + kMaxNumOutputBbox * gpu_bbox_element));
      decode_file.close();
      std::cout << "✅ C++ 解码输出已保存到 bin/cpp_decode_output.bin"
                << std::endl;
    ...
    int img_height = 1440;  // 实际图像高度
    int img_width = 2560;   // 实际图像宽度
    float scale = std::min(kInputH / (float)img_height, kInputW / (float)img_width);
    float dx = (kInputW - scale * img_width) / 2;
    float dy = (kInputH - scale * img_height) / 2;
    
    std::cout << "[DEBUG] GPU坐标变换参数: scale=" << scale << ", dx=" << dx << ", dy=" << dy << std::endl;
    ...
    // 将变换后的坐标复制回GPU内存
    CUDA_CHECK(
        cudaMemcpyAsync(decode_ptr_device, decode_before_nms,
                        sizeof(float) * (1 + kMaxNumOutputBbox * gpu_bbox_element),
    ...
    if (nms_error != cudaSuccess) {
        std::cout << "[ERROR] GPU NMS失败: " << cudaGetErrorString(nms_error) << std::endl;
    } else {
    ...
    CUDA_CHECK(
        cudaMemcpyAsync(decode_ptr_host, decode_ptr_device,
                        sizeof(float) * (1 + kMaxNumOutputBbox * gpu_bbox_element),
                        cudaMemcpyDeviceToHost, stream));
